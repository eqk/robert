...
export class BittrexService {
    ...
    request(method = 'POST', path = '', data = {}) {
        return new Promise((resolve, reject) => {
            if (!data['apikey']) { data['apikey'] = this.config.apiKey; }
            if (!data['nonce']) { data['nonce'] = Date.now() * 1000; }
            ...
            request(options, (err, response, body) => {
                if (response && parseInt(response.statusCode) === 200) {
                    if (body['success']) { resolve({success: true, data: body['result']}); }
                    if (!body['success']) { reject({success: false, errorMsg: body['message'], errorCode: 500}); }
                }
                else {
                    const error = {
                        success: false,
    ...
    balance() {
        return new Promise((resolve) => {
            ...
            this.get('account/getbalances').then(res => {
